<?php
  ...
  if (!isset($_GET['user_id'])) {
    $query = "SELECT user_name, first_name, last_name, gender, birthdate, country FROM forum_users WHERE user_id = '" . $_SESSION['user_id'] . "'";
  }
  else {
    $query = "SELECT user_name, first_name, last_name, gender, birthdate, country FROM forum_users WHERE user_id = '" . $_GET['user_id'] . "'";
  }
  ...
  if (mysqli_num_rows($data) == 1) {
    ...
    if (!empty($row['country'])) {
      echo '<tr><td class="label">Location:</td><td>' . $row['country'] . '</td></tr>';
    }
    echo '</table>';
    if (!isset($_GET['user_id']) || ($_SESSION['user_id'] == $_GET['user_id'])) {
